...
export function Sidebar() {
  ...
  useEffect(() => {
    fetch("/api/v1/console/me", { credentials: "include" }).then((r) => (r.ok ? r.json() : null)).then(setWho).catch(() => undefined);
  }, []);
      ...
      >
        <div className="flex items-center gap-3 px-5 pt-6 pb-5">
          ...
          <div>
            ...
            <div className="label text-[10px] text-ink-faint">STAFF CONSOLE</div>
          </div>
        </div>
        ...
        <div className="px-4 pb-4 pt-2 border-t border-base-line/40">
          <div className="flex items-center gap-3 px-3 py-3 rounded-2xl bg-base shadow-neu-sm">
            <div className="w-9 h-9 rounded-full bg-accent grid place-items-center text-sm font-bold">{(who?.name || "S").slice(0, 1).toUpperCase()}</div>
            <div className="min-w-0">
              <div className="text-sm font-semibold truncate">{who?.name ?? "Staff"}</div>
              <div className="text-xs text-ink-faint truncate">{who?.email ?? "Console access"}</div>
            </div>
          </div>
        </div>
      </aside>
