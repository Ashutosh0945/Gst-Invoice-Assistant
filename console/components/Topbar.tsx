...
import { Bell, LogOut, PanelLeft, RefreshCw, Search, Upload } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-20 bg-base/90 backdrop-blur border-b border-base-line/60">
      <div className="flex items-center gap-3 px-5 md:px-8 h-[72px]">
        ...
        <button aria-label="Sign out" className="icon-btn" onClick={async () => {
          await fetch("/api/v1/app/auth/logout", { method: "POST", credentials: "include" }).catch(() => undefined);
