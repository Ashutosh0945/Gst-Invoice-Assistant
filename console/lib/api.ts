...
function getBaseUrl(): string {
  ...
  return (process.env.BACKEND_URL || "http://localhost:8000").replace(/\/$/, ""); // server: call the API directly
}
...
// Server-side page rendering authenticates to the API with a secret key that never
// reaches the browser (CONSOLE_API_KEY has no NEXT_PUBLIC_ prefix). Who may see a page
// is decided before rendering, by middleware.ts checking the visitor's staff login.
// Browser-side actions (approve, upload…) go through /api with the visitor's own cookie.
function serverAuth(): Record<string, string> {
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  const res = await fetch(`${getBaseUrl()}${path}`, {
    ...init,
    headers: { "Content-Type": "application/json", ...serverAuth(), ...(init?.headers || {}) },
    cache: "no-store",
  });
  if (!res.ok) {
...
function errorText(body: string, status?: number): string {
  // The API answers with JSON {"detail": "..."}; anything else (an HTML error page from the
  // host, an empty body) means the request never reached it properly — say so in plain words.
  try {
    const j = JSON.parse(body);
    if (typeof j.detail === "string") return j.detail;
    if (Array.isArray(j.detail)) return j.detail.map((d: { msg?: string }) => d.msg).filter(Boolean).join(". ");
  } catch { /* not JSON */ }
  if (status === 413) return "That file is too large. Use a file under 4 MB.";
  if (status === 401 || status === 403) return "Your session has ended. Please sign in again.";
  if (status === 504 || status === 408) return "The server took too long to read this file. Try again in a minute.";
  if (status && status >= 500) return "The invoice server couldn't be reached or hit an error. If this keeps happening, check that the API on Render is running (open its /health address).";
